...
NSString *const FMAlertViewWillShowNotification = @"FMAlertViewWillShowNotification";
NSString *const FMAlertViewDidShowNotification = @"FMAlertViewDidShowNotification";
NSString *const FMAlertViewWillDismissNotification = @"FMAlertViewWillDismissNotification";
NSString *const FMAlertViewDidDismissNotification = @"FMAlertViewDidDismissNotification";
...
@end
@interface FMAlertView () <FMViewControllerDelegate>
...
@property (nonatomic, strong) FMAlertView *previousAlert;
...
@property (nonatomic, weak) FMAlertView *alertView;
@property (nonatomic, weak) id <FMViewControllerDelegate> delegate;
...
- (void)showAlert{
    [[NSNotificationCenter defaultCenter] postNotificationName:FMAlertViewWillShowNotification object:self];
    ...
    [UIView animateWithDuration:duration delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:FMAlertViewDidShowNotification object:self.alertView];
    }];
...
- (void)hideAlertWithCompletion:(void(^)(void))completion{
    [[NSNotificationCenter defaultCenter] postNotificationName:FMAlertViewWillDismissNotification object:self];
    self.alertView.alertReady = NO;
    ...
    [UIView animateWithDuration:duration delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:FMAlertViewDidDismissNotification object:self];
    }];
...
+ (void)showOneButtonWithTitle:(NSString *)title Message:(NSString *)message ButtonType:(FMAlertViewButtonType)buttonType ButtonTitle:(NSString *)buttonTitle Click:(clickHandle)click{
    id newClick = click;
    if (!newClick) {
        newClick = [NSNull null];
    }
    FMAlertView *alertView = [FMAlertView new];
...
+ (void)showTwoButtonsWithTitle:(NSString *)title Message:(NSString *)message ButtonType:(FMAlertViewButtonType)
buttonType ButtonTitle:(NSString *)buttonTitle Click:(clickHandle)click ButtonType:(FMAlertViewButtonType)buttonType1 ButtonTitle:(NSString *)buttonTitle1 Click:(clickHandle)click1{
    id newClick = click;
    if (!newClick) {
        newClick = [NSNull null];
    }
...
- (void)dismissAlertWithCompletion:(void(^)(void))completion{
    [self.vc hideAlertWithCompletion:^{
        ...
        NSInteger count = [jCSingleTon shareSingleTon].alertStack.count;
        if (count > 0) {
            FMAlertView *lastAlert = [jCSingleTon shareSingleTon].alertStack.lastObject;
            [lastAlert showAlert];
        }
    }];
}
...
- (void)setButton:(UIButton *)btn BackgroundWithButonType:(FMAlertViewButtonType)buttonType{
    UIColor *textColor = nil;
    UIImage *normalImage = nil;
    UIImage *highImage = nil;
    //可以不断的扩展Button样式
    switch (buttonType) {
        case FMAlertViewButtonTypeDefault:
            ...
            break;
        case FMAlertViewButtonTypeCancel:
            ...
            textColor = WJYColor(255, 255, 255);
            break;
        case FMAlertViewButtonTypeWarn:
            ...
            textColor = WJYColor(255, 255, 255);
            break;
        case FMAlertViewButtonTypeNone:
            
            textColor = [UIColor blueColor];
            [btn setTitleColor:textColor forState:UIControlStateNormal];
            break;
        case FMAlertViewButtonTypeHeight:
