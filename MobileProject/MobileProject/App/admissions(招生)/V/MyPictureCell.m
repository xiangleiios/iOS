...
//        }];
    ...
    self.time = lbtwo;
    self.time.textColor = kColor_N(180, 181, 201);
    lbtwo.font = [UIFont systemFontOfSize:kFit_Font6(13)];
    [lbtwo mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.imgbavk.mas_bottom).mas_offset(KFit_H6S(30));
        make.left.mas_equalTo(self.contentView).mas_offset(KFit_W6S(30));
        make.size.mas_equalTo(CGSizeMake(KFit_W6S(220), KFit_H6S(45)));
    }];
    
    self.num = [[UIButton alloc] init];
    [self.contentView addSubview:self.num];
    [self.num setTitle:@"分享" forState:UIControlStateNormal];
    self.num.titleLabel.font = [UIFont systemFontOfSize:kFit_Font6(15)];
    [self.num setBackgroundImage:[UIImage createImageWithColor:kColor_N(0, 112, 234)] forState:UIControlStateNormal];
    [self.num addTarget:self action:@selector(toShare) forControlEvents:UIControlEventTouchUpInside];
    [self.num setBackgroundImage:[UIImage createImageWithColor:kRGBAColor(0, 112, 234, 0.6)] forState:UIControlStateHighlighted];
    [self.num mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.mas_equalTo(self.time);
        make.right.mas_equalTo(self.contentView).mas_offset(-KFit_W6S(30));
        make.size.mas_equalTo(CGSizeMake(KFit_W6S(150), KFit_H6S(50)));
    }];
...
- (void)setModel:(FMMainModel *)model{
    ...
    self.time.text = [XLCommonUse TimeToIntercept:_model.createTime];
