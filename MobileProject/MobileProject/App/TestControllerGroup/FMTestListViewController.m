...
#import "FMTestListViewController.h"

#import "FMWebShareViewController.h"
#import "FMLoginViewController.h"
#import "FMSearchBarViewController.h"

#import "FMShare.h"

@interface FMTestListViewController()<UITableViewDataSource, UITableViewDelegate>
@property (nonatomic,strong) NSArray             *dataArray;
@property (nonatomic,strong) UITableView         *myTableView;
@end

@implementation FMTestListViewController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.row) {
        case 0:
        {
            FMWebShareViewController *vc=[[FMWebShareViewController alloc]init];
        ...
        case 2:
        {
            ...
            FMShareMenuView *shareView = [[FMShareMenuView alloc] init];
        ...
        case 3:
        {
            FMSearchBarViewController *vc=[[FMSearchBarViewController alloc]init];
            [self.navigationController pushViewController:vc animated:YES];
            break;
        }
        case 4:
        {
            [FMAlertView showTwoButtonsWithTitle:@"提示信息" Message:@"这里为提示的信息内容，里面会根据内容的高度进行计算，当大于弹出窗默认的高度时会自行适应高度(并自动转成UITextView来加载内容)" ButtonType:FMAlertViewButtonTypeNone ButtonTitle:@"取消" Click:^{
                NSLog(@"您点取消事件");
            } ButtonType:FMAlertViewButtonTypeNone ButtonTitle:@"确定" Click:^{
                NSLog(@"你点确定事件");
            }];
