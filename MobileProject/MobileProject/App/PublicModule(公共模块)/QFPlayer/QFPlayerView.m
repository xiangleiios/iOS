...
static const CGFloat QFPlayerAnimationTimeInterval             = 7.0f;
static const CGFloat QFPlayerControlBarAutoFadeOutTimeInterval = 0.5f;
...
//播放器的几种状态
typedef NS_ENUM(NSInteger, QFPlayerState) {
    QFPlayerStateFailed,     // 播放失败
    QFPlayerStateBuffering,  //缓冲中
    QFPlayerStatePlaying,    //播放中
    QFPlayerStateStopped,    //停止播放
    QFPlayerStatePause       //暂停播放
};

@interface QFPlayerView () <UIGestureRecognizerDelegate, UIAlertViewDelegate>
...
/** 播发器的几种状态 */
@property (nonatomic, assign) QFPlayerState       state;
/** 控制层View */
@property (nonatomic, retain) QFPlayerControlView *controlView;
/** 是否为全屏 */
...
{
    static QFPlayerView *playerView = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        playerView = [[QFPlayerView alloc] init];
    });
    return playerView;
}
...
- (void)addNotifications
{
    ...
    self.controlView.titleLabel.text = self.name;
    // slider开始滑动事件
    ...
    // cell上播放视频的话，该返回按钮为×
    if (self.isCellVideo) {
        [self.controlView.backBtn setImage:[UIImage imageNamed:QFPlayerSrcName(@"kr-video-player-close")] forState:UIControlStateNormal];
    }else {
        [self.controlView.backBtn setImage:[UIImage imageNamed:@"back"] forState:UIControlStateNormal];
    }
    // 返回按钮点击事件
...
- (void)setVideoURL:(NSURL *)videoURL
{
    ...
    if ([[QFDownloadManager sharedInstance] isCompletion:videoURL.absoluteString]) {
        videoURL = [NSURL fileURLWithPath:QFFileFullpath(videoURL.absoluteString)];
    }
    _videoURL = videoURL;
    
    self.loadingImage = [[UIImageView alloc] init];
    [self.loadingImage sd_setImageWithURL:[NSURL URLWithString:self.imageUri]];
...
- (void)configZFPlayer
{
    ...
    if ([self.videoURL.scheme isEqualToString:@"file"]) {
        self.state = QFPlayerStatePlaying;
        self.isLocalVideo = YES;
        self.controlView.downLoadBtn.enabled = NO;
    } else {
        self.state = QFPlayerStateBuffering;
...
- (void)autoFadeOutControlBar
{
    ...
    [self performSelector:@selector(hideControlView) withObject:nil afterDelay:QFPlayerAnimationTimeInterval];
...
- (void)hideControlView
{
    ...
    [UIView animateWithDuration:QFPlayerControlBarAutoFadeOutTimeInterval animations:^{
        [self.controlView hideControlView];
        if (self.isFullScreen) { //全屏状态
            self.controlView.titleLabel.alpha = 0;
            ...
            self.controlView.backBtn.alpha = 1;//0
            self.controlView.titleLabel.hidden = 0;
        }
...
- (void)animateShow
{
    ...
    [UIView animateWithDuration:QFPlayerControlBarAutoFadeOutTimeInterval animations:^{
        self.controlView.backBtn.alpha = 1;
        self.controlView.titleLabel.alpha = 1;
    ...
    if (object == self.player.currentItem) {
        if ([keyPath isEqualToString:@"status"]) {
            if (self.player.currentItem.status == AVPlayerItemStatusReadyToPlay) {
                ...
                self.state = QFPlayerStateFailed;
            ...
            // 当缓冲好的时候
            if (self.playerItem.playbackLikelyToKeepUp && self.state == QFPlayerStateBuffering){
                self.state = QFPlayerStatePlaying;
            }
            
        }
    }else if (object == self.tableView) {
        if ([keyPath isEqualToString:kQFPlayerViewContentOffset]) {
...
- (void)setOrientationLandscape
{
    if (self.isCellVideo) {
        // 横屏时候移除tableView的观察者
        [self.tableView removeObserver:self forKeyPath:kQFPlayerViewContentOffset];
        [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleLightContent animated:YES];
        // 亮度view加到window最上层
        QFBrightnessView *brightnessView = [QFBrightnessView sharedBrightnessView];
...
//    [[UIDevice currentDevice] setValue:value forKey:@"orientation"];
    ...
    switch (interfaceOrientation) {
        ...
        case UIInterfaceOrientationPortraitUpsideDown:{
            QFPlayerShared.isAllowLandscape = NO;
            [self interfaceOrientation:UIInterfaceOrientationPortrait];
        }
            break;
        case UIInterfaceOrientationPortrait:{
            QFPlayerShared.isAllowLandscape = YES;
...
- (void)onDeviceOrientationChange
{
    ...
    switch (interfaceOrientation) {
        case UIInterfaceOrientationPortraitUpsideDown:{
            ...
            if (self.isCellVideo) {
                [self.controlView.backBtn setTitle:_name forState:UIControlStateNormal];
                [self.controlView.backBtn setImage:[UIImage imageNamed:@"back"] forState:UIControlStateNormal];
            }
            // 设置返回按钮的约束
        ...
        case UIInterfaceOrientationPortrait:{
            ...
            if (self.isCellVideo) {
                ...
                if (!self.isFullScreen) {
                    ...
                    // 重新监听tableview偏移量
                    [self.tableView addObserver:self forKeyPath:kQFPlayerViewContentOffset options:NSKeyValueObservingOptionNew context:nil];
                }
                // 当设备转到竖屏时候，设置为竖屏约束
                [self setOrientationPortrait];
    ...
    if (self.isCellVideo && !QFPlayerShared.isAllowLandscape) {
        [self.controlView.backBtn setImage:[UIImage imageNamed:QFPlayerSrcName(@"kr-video-player-close")] forState:UIControlStateNormal];
        [self.controlView.backBtn mas_updateConstraints:^(MASConstraintMaker *make) {
            make.top.leading.mas_equalTo(5);
        }];
...
- (void)lockScreenAction:(UIButton *)sender
{
    ...
    QFPlayerShared.isLockScreen = sender.selected;
}
...
- (void)unLockTheScreen
{
    ...
    QFPlayerShared.isLockScreen       = NO;
...
- (void)startAction:(UIButton *)button
{
    ...
    if (button.selected) {
        [self play];
        if (self.state == QFPlayerStatePause) { self.state = QFPlayerStatePlaying; }
    } else {
        [self pause];
        if (self.state == QFPlayerStatePlaying) { self.state = QFPlayerStatePause;}
    }
}
...
- (void)downloadVideo:(UIButton *)sender
{
    ...
    [[QFDownloadManager sharedInstance] download:self.videoURL.absoluteString progress:^(CGFloat progress, NSString *speed, NSString *remainingTime, NSString *writtenSize, NSString *totalSize) {
...
{
    self.state            = QFPlayerStateStopped;
 ...
 *  设置播放的状态
 *
 *  @param state QFPlayerState
 */
- (void)setState:(QFPlayerState)state
{
    _state = state;
    if (state == QFPlayerStatePlaying) {
        // 改为黑色的背景，不然站位图会显示
        UIImage *image = [self buttonImageFromColor:[UIColor blackColor]];
        self.layer.contents = (id) image.CGImage;
    } else if (state == QFPlayerStateFailed) {
        self.controlView.downLoadBtn.enabled = NO;
    }
    // 控制菊花显示、隐藏
    state == QFPlayerStateBuffering ? ([self.controlView.activity startAnimating]) : ([self.controlView.activity stopAnimating]);
}
...
- (void)setTableView:(UITableView *)tableView
{
    ...
    if (_tableView) { [_tableView removeObserver:self forKeyPath:kQFPlayerViewContentOffset]; }
    _tableView = tableView;
    if (tableView) { [tableView addObserver:self forKeyPath:kQFPlayerViewContentOffset options:NSKeyValueObservingOptionNew context:nil];}
}
 ...
 */
- (void)setPlayerLayerGravity:(QFPlayerLayerGravity)playerLayerGravity
{
    ...
    switch (playerLayerGravity) {
        case QFPlayerLayerGravityResize:
            self.playerLayer.videoGravity = AVLayerVideoGravityResizeAspect;
            break;
        case QFPlayerLayerGravityResizeAspect:
            self.playerLayer.videoGravity = AVLayerVideoGravityResizeAspect;
            break;
        case QFPlayerLayerGravityResizeAspectFill:
 ...
 * 懒加载 控制层View
 *
 *  @return QFPlayerControlView
 */
- (QFPlayerControlView *)controlView
{
    if (!_controlView) {
        _controlView = [[QFPlayerControlView alloc] init];
